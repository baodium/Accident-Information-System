...
<?php foreach($routes as $rt){ ?>
   ...
   <?php } ?>    
    </select><input class="btn-standard"  style=" background:#120338; color:#FFFFFF" type="submit"  name="view_route" value="View Information" title="view route information" ></div></form>
  </aside>
<br/><br/>
  </section>        
...
<article class="conv-message mine" id="message_34686812">
      ...
      <table style="background-color:#FFFFFF; color:#000000; margin-left:-82px; border-color:#FFFFFF "  width="850px" border="2px"  bordercolor="#FFFFFF">
         ...
         <tr style=" background: #003366; height:30px;  color: #fff; font-size:12px;    "  ><th>&nbsp;&nbsp;</th><th>Route Name</th><th>Lenght</th><th>Carriage Width</th><th>Pot Holes</th><th>Surface</th><th>Failed Segment</th><th>Junctions </th><th>Bends</th><th>View Full Detail</th></tr>
<?php $i=0;
foreach($route_list as $rt) {  $i++; ?>
<tr style=" height:25px; padding-top:10px" class="<?php echo ($i%2==0)?'odd':''  ?>" ><td><center><input type="checkbox" name="select[]" onClick="tick();" value="<?php echo $rt['route_id'] ?>"></center></td><td>&nbsp;<?php echo $rt['route_name'] ?></td><td><center><?php echo $rt['route_length'] ?>Km</center></td><td><center><?php echo $rt['carriage_width'] ?>M</center></td><td><center><?php echo  $rt['pot_holes'] ?></center></td><td><center><?php echo $rt['surface'] ?></center></td><td><center><?php echo $rt['failed_segment'] ?></center></td><td><center><?php echo $rt['junctions'] ?></center></td><td><center><?php echo $rt['bends'] ?></center></td><td><center><a href="info.php?id=<?php echo $rt['route_id'] ?>"><img src="../images/save.png" /></a></center></td></tr>
<?php } ?> 
         </table><br/><br/>
         <?php 
